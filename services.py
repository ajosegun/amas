...
def bulksmsnigeria_module(phone_number, sender_name, message):
    my_api_token = config.bulksmsnigeria_moduleAPI
    ...
    params = {
    'api_token': config.bulksmsnigeria_moduleAPI,
    'to': phone_number,
    'from': 'sender_name',
    'body': message,
    'dnd': 'ut',
...
def smsclone_module(phone_number, sender_name, message):
    ...
    url = url.format(_username = config.smsclone_username, _password = config.smsclone_password, _sender= 'AMAS', _recipient = config.smsclone_recipient, _message=message)
